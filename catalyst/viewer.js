...
const controls=new OrbitControls(camera,renderer.domElement);controls.enableDamping=true;controls.dampingFactor=.06;controls.minDistance=4.5;controls.maxDistance=200;controls.maxPolarAngle=Math.PI*.49;controls.minPolarAngle=.23;controls.autoRotateSpeed=.65;controls.target.set(0,2.07,0);
function resetCamera(){const mobile=innerWidth<800;camera.position.set(mobile?-5.0:-5.2,mobile?4.0:3.65,mobile?26:17);controls.target.set(0,mobile?3.2:3.1,0);if(mobile){camera.setViewOffset(innerWidth,innerHeight,0,innerHeight*.18,innerWidth,innerHeight)}else{camera.setViewOffset(innerWidth,innerHeight,innerWidth*.065,0,innerWidth,innerHeight)}controls.update()}
resetCamera();
...
function setMotion(name){const next=mixer.clipAction(robot.clips.find(c=>c.name===name));if(next!==current){next.reset().setEffectiveTimeScale(1).setEffectiveWeight(1).play();current.crossFadeTo(next,.4,true);current=next}motion=name;status.textContent={Idle:'のんびり待機中',Walk:'てくてく散歩中',Wave:'こんにちは！',Look:'あたりを見回しています',Guard:'天秤を掲げ、委任の重みを受け止めます'}[name];document.querySelectorAll('[data-motion]').forEach(b=>{b.classList.toggle('active',b.dataset.motion===name);b.setAttribute('aria-pressed',String(b.dataset.motion===name))})}
...
const TIER_LABEL={wood:'木',bronze:'銅',silver:'銀',gold:'金',platinum:'プラチナ',diamond:'ダイヤ'};
// Slider 0-100 maps log-style onto $0 - $4M so the lower tiers get real travel.
const sliderToUsd=v=>v<=0?0:Math.round(10000*Math.pow(400,v/100)/1000)*1000;
const usdToSlider=usd=>usd<=0?0:Math.round(100*Math.log(usd/10000)/Math.log(400));
const fmtUsd=v=>v<=0?'$0':v>=1e6?'$'+(v/1e6).toFixed(v>=3e6?1:2)+'M':'$'+Math.round(v/1e3)+'万'.replace('万','K');
function updateFunding(){const s=robot.ward.state;
 $('#total').value=usdToSlider(s.totalUsd);$('#total-value').textContent=fmtUsd(s.totalUsd);
 $('#rounds').value=s.rounds;$('#rounds-value').textContent=s.rounds+'回';
 $('#tier-name').textContent=TIER_LABEL[s.tier]+'ティア';
 $('#tier-caption').textContent='累計 '+fmtUsd(s.totalUsd)+' / '+s.rounds+'回の採択。'+(s.tier==='diamond'?'背負い切れない分は袋に。':s.tier==='wood'?'最初の一件から始まります。':'コイン1枚 ≈ $10万。');}
$('#total').oninput=e=>{robot.ward.setState({totalUsd:sliderToUsd(Number(e.target.value))});updateFunding()};
$('#rounds').oninput=e=>{robot.ward.setState({rounds:Number(e.target.value)});updateFunding()};
document.querySelectorAll('[data-preset]').forEach(b=>b.onclick=()=>{const p=b.dataset.preset;
 robot.ward.setState(p==='first'?{rounds:1,totalUsd:30000}:p==='mid'?{rounds:3,totalUsd:180000}:p==='veteran'?{rounds:7,totalUsd:600000}:{rounds:22,totalUsd:2800000});
 pressed('[data-preset]',b);updateFunding()});
$('#lantern').closest('label').hidden=true;updateFunding();
