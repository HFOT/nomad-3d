...
export class PlayerMotion {
 ...
 jump(){if(this.grounded){this.vy=7.6;this.grounded=false;this.jumping=true;return true;}return false;}
 update(position,input,dt,world){
  const steps=Math.max(1,Math.ceil(dt/(1/60))),h=dt/steps;
  for(let i=0;i<steps;i++){
   const wanted=input.moving?(input.sprint?11.5:6):0;
   this.speed+= (wanted-this.speed)*(1-Math.exp(-h*14));
   ...
   if(!this.grounded){
    this.vy-=20*h;const nextY=position.y+this.vy*h;
