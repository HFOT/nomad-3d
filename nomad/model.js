...
export function createMaterials(texSize){
  ...
  return {
    ivory:make('Aged warm porcelain','#d4c8a6',.2,.46,'ivory'),teal:make('Chipped petrol enamel','#355854',.62,.43,'paint'),darkTeal:make('Deep petrol trim','#213e3c',.65,.42,'paint'),
    ...
    cloth:make('Woven ochre scarf','#aa751e',0,.98,'cloth'),clothDark:make('Ochre seams','#795419',0,1),leather:make('Weathered brown leather','#51372c',0,.82,'leather'),leatherLight:make('Backpack flap leather','#684838',0,.79,'leather'),thread:make('Linen stitching','#ae9570',0,.95),
...
export function buildRobot(M){
  const root=new T.Group();root.name='NOMAD';const rig={};let serial=0;
  ...
  const body=rig.body=group(root,'Body',0,1.75,0);
  ...
  const torso=group(body,'Torso',0,.56,0);
  lathe(torso,M.teal,[[0,-.49],[.39,-.49],[.54,-.42],[.61,-.25],[.61,.1],[.56,.3],[.44,.49],[.25,.53],[0,.53]],0,0,0,1,.77);
  ...
  const head=rig.head=group(body,'Head',0,1.86,.01);
  lathe(head,M.ivory,[[0,-.65],[.47,-.65],[.68,-.60],[.80,-.47],[.87,-.23],[.88,.05],[.83,.34],[.69,.58],[.44,.74],[.12,.81],[0,.815]],0,0,0,1,.88);
  ...
  for(const s of [-1,1]){
    const e=group(head,'Optic_'+(s<0?'L':'R'),s*.355,.00,.706);e.rotation.y=s*.16;
    ...
    curve(e,M.ivory,[[-.20,.07,.02],[-.13,.205,.035],[0,.239,.035],[.13,.205,.035],[.20,.07,.02]],.02);
  }
  const mouth=box(head,M.dark,0,-.335,.750,.18,.045,.03,.015);mouth.rotation.x=.18;
  box(head,M.brass,0,-.365,.743,.16,.009,.018,.002);
  for(const s of [-1,1]){
  ...
  const antenna=rig.antenna=group(head,'Antenna',.08,.795,-.12);
  ...
  const scarf=group(body,'Scarf',0,1.13,.015);
  const collarV=[],collarUV=[],collarI=[];
  for(let i=0;i<=96;i++)for(let j=0;j<=16;j++){const a=i/96*Math.PI*2,v=j/16,r=.40+.048*Math.sin(v*Math.PI)+.018*Math.sin(a*9+v*5)+.008*Math.sin(a*21-v*8);collarV.push(Math.cos(a)*r*1.14,-.065+v*.29+.025*Math.sin(a*3+v*4),Math.sin(a)*r*.9);collarUV.push(i/96*2,v)}
  ...
  for(const s of [-1,1]){
    const leg=rig['leg'+s]=group(root,'Hip_'+s,s*.32,1.64,0);axle(leg,0,0,0,.17);
    ...
    const ankle=rig['foot'+s]=group(knee,'Foot_'+s,0,-.68,.025);ankle.scale.set(1.15,1,1.1);cyl(ankle,M.brass,0,.03,0,.135,.15,'x');
    box(ankle,M.rubber,0,-.28,.13,.48,.12,.73,.075);box(ankle,M.leather,0,-.21,.13,.47,.05,.715,.035);
    sphere(ankle,M.teal,0,-.12,.14,.239,.174,.365);
    const toeGeo=new T.SphereGeometry(1,40,20,0,Math.PI*2,0,1.14);toeGeo.rotateX(Math.PI/2);const toe=mesh(ankle,toeGeo,M.ivory,0,-.119,.14);toe.scale.set(.242,.178,.371);
  ...
  for(const s of [-1,1]){
    const arm=rig['arm'+s]=group(body,'Shoulder_'+s,s*.67,.87,0);axle(arm,0,0,0,.22);
    sphere(arm,M.iron,0,-.09,0,.17,.2,.18);
    const armor=sphere(arm,M.ivory,s*.065,-.28,.025,.176,.255,.183);armor.rotation.z=s*.10;
  ...
  root.userData={title:'NOMAD — Field companion 01',author:'CORN',units:'meters',animation:'Rigid hierarchical articulation, four baked looping clips'};
  ...
  function pose(mode,t){
    ...
    rig.antenna.rotation.z=.03*Math.sin(t*Math.PI);
    // Counter-rotate lantern so gravity remains vertical despite the articulated arm.
    root.updateMatrixWorld(true);const q=new T.Quaternion();rig.hand1.getWorldQuaternion(q);lantern.quaternion.copy(q.invert());lantern.rotateZ((walk?.12:.035)*Math.sin(t*Math.PI));lantern.rotateX(.028*Math.sin(t*Math.PI+.8));
  }
  ...
  return {root,rig,clips,pose,helpers:{mesh,sphere,box,cyl,torus,curve,rod}};
}
