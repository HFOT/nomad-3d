...
$viewerReady = $false
try { $viewerResponse = Invoke-WebRequest -Uri $viewerUrl -UseBasicParsing -TimeoutSec 2; $viewerReady = $viewerResponse.Content.Contains('NOMAD') } catch {}
if (-not $viewerReady) {
    ...
    for ($viewerAttempt=0; $viewerAttempt -lt 30; $viewerAttempt++) {
        ...
        try { $viewerResponse = Invoke-WebRequest -Uri $viewerUrl -UseBasicParsing -TimeoutSec 1; if ($viewerResponse.Content.Contains('NOMAD')) { $viewerReady=$true; break } } catch {}
    }
}
