...
export function buildVault(){
 ...
 function batch(group){group.updateMatrixWorld(true);const groups=new Map();group.traverse(o=>{if(!o.isMesh)return;const g=o.geometry.clone();g.applyMatrix4(o.matrixWorld);if(!groups.has(o.material))groups.set(o.material,[]);const parts=groups.get(o.material);if(g.index){const flat=g.toNonIndexed();g.dispose();parts.push(flat);}else parts.push(g);});group.clear();for(const [m,gs] of groups){const merged=mergeGeometries(gs,false);if(merged){const o=new T.Mesh(merged,m);o.castShadow=o.receiveShadow=true;group.add(o);}gs.forEach(g=>g.dispose());}}
