...
const fmtAda=v=>v<=0?'0 ADA':v.toFixed(v<10?1:1).replace(/\.0$/,'')+'M ADA';
function updateVault(){const s=robot.ward.state;
 $('#energy').value=s.energyM;$('#energy-unit').textContent=(s.energyM<=0?'0':String(Math.round(s.energyM)))+'M';
 $('#rounds').value=s.rounds;$('#rounds-value').textContent=s.rounds+'回';
 $('#energy-value').textContent=fmtAda(s.energyM);
 $('#energy-caption').textContent=s.energyM<=0&&s.rounds<=0?'まだ何も引き出されていません。DRepの声を待っています。':'蓄え '+fmtAda(s.energyM)+' / 承認 '+s.rounds+'回。'+(s.energyM>=67?'吸気口は最高段階。装甲はレインボーへ。':s.energyM>=34?'吸気口が高度化。ギアが唸り始めた。':'ギアは静かに回り続けています。');}
$('#energy').oninput=e=>{robot.ward.setState({energyM:Number(e.target.value)});updateVault()};
$('#rounds').oninput=e=>{robot.ward.setState({rounds:Number(e.target.value)});updateVault()};
document.querySelectorAll('[data-preset]').forEach(b=>b.onclick=()=>{const p=b.dataset.preset;
 robot.ward.setState(p==='quiet'?{rounds:0,energyM:0}:p==='first'?{rounds:1,energyM:10}:p==='budget'?{rounds:4,energyM:50}:{rounds:9,energyM:97});
 pressed('[data-preset]',b);updateVault()});
$('#lantern').closest('label').hidden=true;updateVault();
