...
function resetCamera(){const mobile=innerWidth<800;camera.position.set(mobile?-5.0:-5.2,mobile?4.0:3.65,mobile?26:15.6);controls.target.set(0,mobile?3.15:2.95,0);if(mobile){camera.setViewOffset(innerWidth,innerHeight,0,innerHeight*.18,innerWidth,innerHeight)}else{camera.setViewOffset(innerWidth,innerHeight,innerWidth*.065,0,innerWidth,innerHeight)}controls.update()}
resetCamera();
...
function setMotion(name){const next=mixer.clipAction(robot.clips.find(c=>c.name===name));if(next!==current){next.reset().setEffectiveTimeScale(1).setEffectiveWeight(1).play();current.crossFadeTo(next,.4,true);current=next}motion=name;status.textContent={Idle:'のんびり待機中',Walk:'てくてく散歩中',Wave:'こんにちは！',Look:'あたりを見回しています',Guard:'憲法書と提案を照合します'}[name];document.querySelectorAll('[data-motion]').forEach(b=>{b.classList.toggle('active',b.dataset.motion===name);b.setAttribute('aria-pressed',String(b.dataset.motion===name))})}
...
$('#capture').onclick=()=>{composer.render();renderer.domElement.toBlob(b=>download(b,'LEX-portrait.png'))};
...
$('#export').onclick=async()=>{try{download(new Blob([await exportModel()],{type:'model/gltf-binary'}),'LEX-animated.glb')}catch(e){console.error(e);status.textContent='保存に失敗しました。もう一度お試しください'}};
...
function syncCC(){const s=robot.ward.state;document.querySelectorAll('[data-vote]').forEach(b=>{b.disabled=!s.confidence||s.term===0;b.classList.toggle('active',b.dataset.vote===s.vote);b.setAttribute('aria-pressed',String(b.dataset.vote===s.vote))});$('#confidence').checked=!s.confidence;$('#term').value=s.term;$('#term-value').value=s.term+'%';$('#cc-status').textContent=!s.confidence?'NO CONFIDENCE · 委員会更新待ち':s.term===0?'任期満了 · この委員は投票できません':{review:'REVIEW · 憲法と照合中',yes:'YES · 合憲と判断',no:'NO · 違憲と判断',abstain:'ABSTAIN · 棄権'}[s.vote];$('#cc-status').classList.toggle('alert',!s.confidence||s.term===0);}
document.querySelectorAll('[data-vote]').forEach(b=>b.onclick=()=>{robot.ward.setState({vote:b.dataset.vote});syncCC()});$('#confidence').onchange=e=>{robot.ward.setState({confidence:!e.target.checked});syncCC()};$('#term').oninput=e=>{robot.ward.setState({term:Number(e.target.value)});syncCC()};$('#renew').onclick=()=>{robot.ward.setState({confidence:true,term:100,vote:'review'});syncCC()};$('#lantern').closest('label').hidden=true;syncCC();
