...
export function createMaterials(texSize){
  ...
  return {
    ivory:make('Aged warm porcelain','#d4c8a6',.2,.46,'ivory'),teal:make('Chipped petrol enamel','#263f60',.62,.43,'paint'),darkTeal:make('Deep petrol trim','#1e3048',.65,.42,'paint'),
    ...
    cloth:make('Woven ochre scarf','#263448',0,.98,'cloth'),clothDark:make('Ochre seams','#172335',0,1),leather:make('Weathered brown leather','#51372c',0,.82,'leather'),leatherLight:make('Backpack flap leather','#684838',0,.79,'leather'),thread:make('Linen stitching','#ae9570',0,.95),
...
export function buildRobot(M){
  const root=new T.Group();root.name='WARD';const rig={};let serial=0;
  ...
  root.userData={title:'WARD — Field companion 01',author:'CORN',units:'meters',animation:'Rigid hierarchical articulation, four baked looping clips'};
