...
const controls=new OrbitControls(camera,renderer.domElement);controls.enableDamping=true;controls.dampingFactor=.06;controls.minDistance=4.5;controls.maxDistance=28;controls.maxPolarAngle=Math.PI*.49;controls.minPolarAngle=.23;controls.autoRotateSpeed=.65;controls.target.set(0,2.07,0);
function resetCamera(){const mobile=innerWidth<800;camera.position.set(mobile?-5.0:-5.2,mobile?4.0:3.65,mobile?20.8:10.9);controls.target.set(0,mobile?2.50:2.30,0);if(mobile){camera.setViewOffset(innerWidth,innerHeight,0,innerHeight*.18,innerWidth,innerHeight)}else{camera.setViewOffset(innerWidth,innerHeight,innerWidth*.065,0,innerWidth,innerHeight)}controls.update()}
resetCamera();
...
function setMotion(name){const next=mixer.clipAction(robot.clips.find(c=>c.name===name));if(next!==current){next.reset().setEffectiveTimeScale(1).setEffectiveWeight(1).play();current.crossFadeTo(next,.4,true);current=next}motion=name;status.textContent={Idle:'のんびり待機中',Walk:'てくてく散歩中',Wave:'こんにちは！',Look:'あたりを見回しています',Guard:'盾を構えてチェーンを守ります'}[name];document.querySelectorAll('[data-motion]').forEach(b=>{b.classList.toggle('active',b.dataset.motion===name);b.setAttribute('aria-pressed',String(b.dataset.motion===name))})}
...
$('#capture').onclick=()=>{composer.render();renderer.domElement.toBlob(b=>download(b,'WARD-portrait.png'))};
...
$('#export').onclick=async()=>{try{download(new Blob([await exportModel()],{type:'model/gltf-binary'}),'WARD-animated.glb')}catch(e){console.error(e);status.textContent='保存に失敗しました。もう一度お試しください'}};
...
window.ward={scene,robot,mixer,camera,controls,renderer,composer,setMotion,exportModel,lighting,get state(){return{motion,surface,paused,frames,drawCalls:renderer.info.render.calls,triangles:renderer.info.render.triangles}}};
function relayUI(){
  const s=robot.ward.state;$('#health').value=Math.round(s.health*100);$('#health-value').value=Math.round(s.health*100)+'%';$('#latency').value=s.latency;$('#latency-value').value=s.latency+'ms';$('#shared').checked=s.shared;
  $('#relay-caption').textContent=s.health>.98?(s.shared?'3灯は正常に点灯。配管は一つの共通経路へ集約します。':'3灯が点灯。それぞれの配管でつながっています。'):(s.shared?'到達・同期が低下。共通経路への依存もある状態です。':'灯と盾の接続が部分的に弱まっています。');
}
document.querySelectorAll('[data-preset]').forEach(b=>b.onclick=()=>{const p=b.dataset.preset;robot.ward.setState({health:p==='degraded'?.32:1,shared:p==='shared',latency:p==='degraded'?650:120});pressed('[data-preset]',b);relayUI()});
function customRelay(){robot.ward.setState({health:Number($('#health').value)/100,latency:Number($('#latency').value),shared:$('#shared').checked});document.querySelectorAll('[data-preset]').forEach(b=>{b.classList.remove('active');b.setAttribute('aria-pressed','false')});relayUI()}
$('#health').oninput=customRelay;$('#latency').oninput=customRelay;$('#shared').onchange=customRelay;
// The old lantern checkbox is a master relay illumination switch in this sibling.
$('#lantern').closest('label').hidden=true;
relayUI();
