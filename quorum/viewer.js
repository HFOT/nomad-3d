...
function updatePower(){const s=robot.ward.state;$('#power').value=s.power;$('#concentration').value=s.concentration;$('#concentration-value').value=s.concentration+'%';$('#relay-caption').textContent='上位10 DRepの合計 '+s.concentration+'% / 目標 ≤15%。'+(s.concentration>15?'目標線を超えても成長は続きます。':'分散化目標の範囲です。');$('#growth-value').textContent=s.power.toLocaleString(undefined,{maximumFractionDigits:1})+' ×';$('#warning').textContent=s.concentration>15?'⚠ TARGET15超過 / 警告中も成長を制限しません':'TARGET15内 / 上位10合計 ≤15%';$('#warning').classList.toggle('alert',s.concentration>15);}
$('#power').onchange=e=>{robot.ward.setState({power:e.target.value});updatePower()};
$('#grow').onclick=()=>{robot.ward.setState({power:robot.ward.state.power*2});updatePower()};
$('#release').onclick=()=>{robot.ward.setState({power:robot.ward.state.power/2});updatePower()};
$('#concentration').oninput=e=>{robot.ward.setState({concentration:e.target.value});updatePower()};
document.querySelectorAll('[data-preset]').forEach(b=>b.onclick=()=>{const p=b.dataset.preset;robot.ward.setState(p==='distributed'?{power:1,concentration:15}:p==='growing'?{power:4,concentration:32}:{power:16,concentration:60});pressed('[data-preset]',b);updatePower()});
$('#lantern').closest('label').hidden=true;updatePower();

setInterval(()=>{if($('#inflow').checked&&!paused){robot.ward.setState({power:robot.ward.state.power*1.08});updatePower()}},500);
